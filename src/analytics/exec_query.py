...
def date_range(start, stop, monthly=False):
    ...
    while start <= stop:
        dates.append(start)
        dt_start = datetime.datetime.strptime(start, "%Y-%m-%d") + datetime.timedelta(
            days=1
        )
...
def exec_query(table, db_origin, db_target, dt_start, dt_stop, monthly):
    ...
    for i in tqdm(dates):
        ...
        with engine_analytical.connect() as con:
            try:
                query_delete = (
                    f"DELETE FROM {table} WHERE dtRef = date('{i}', '-1 day')"
                )
...
def main():
    ...
    exec_query(
        args.table, args.db_origin, args.db_target, args.start, args.stop, args.monthly
    )
