...
tb_idade AS (
    SELECT IdCliente,
        cast(
            max(julianday('{date}') - julianday(dtDia)) as int
        ) AS qtdeDiasPrimTransacao,
        cast(
            min(julianday('{date}') - julianday(dtDia)) as int
        ) AS qtdeDiasUltTransacao
    FROM tb_daily
    GROUP BY IdCliente
),
tb_rn AS (
    ...
    SELECT IdCliente,
        ...
        sum(
            CASE
                ...
                ELSE 0
            END
        ) as qtdePontosPos -- sum(abs(QtdePontos)) as qtdePontosAbs
    FROM transacoes
    WHERE DtCriacao < '{date}'
