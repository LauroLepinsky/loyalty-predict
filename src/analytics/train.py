...
imput_new = imputation.CategoricalImputer(
    fill_value="Nao-Usuario", variables=["descLifeCycleD28"]
)

imput_1000 = imputation.ArbitraryNumberImputer(
    arbitrary_number=1000,
    variables=["avgIntervaloDiasVida", "avgIntervaloDiasD28", "qtdDiasUltiAtividade"],
)
...
grid = model_selection.GridSearchCV(
    model, param_grid=params, cv=3, scoring="roc_auc", refit=True, verbose=3, n_jobs=-1
)
...
feature_importance = pd.Series(
    model_pipeline[-1].feature_importances_, index=features_names
)
