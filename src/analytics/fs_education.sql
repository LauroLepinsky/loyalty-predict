...
tb_pct_cursos AS (
    ...
    FROM tb_usuario_cursos AS t1
        LEFT JOIN tb_cursos_total_eps AS t2 ON t1.descSlugCurso = t2.descSlugCurso
),
tb_pct_cursos_pivot AS (
    SELECT idUsuario,
        ...
        SUM(
            CASE
                WHEN pctCursoCompleto > 0
                and pctCursoCompleto < 1 then 1
                else 0
            end
        ) AS qtdeCursosIncompletos,
        SUM(
            CASE
...
(
    SELECT IdUsuario,
            descSlugCurso,
            COUNT(descSlugCurso) AS qtdeEps
    FROM cursos_episodios_completos
...
FROM tb_atividade
GROUP BY idUsuario
>>>>>>> parent of c613d81 (dia 06 feature store ciclo de vida e rfv)
),
tb_join AS (
<<<<<<< HEAD
